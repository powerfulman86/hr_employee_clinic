...
class Warehouse(models.Model):
    ...
    clinic_type_id = fields.Many2one('stock.picking.type', string="Clinic Operation Type")

    def _get_sequence_values(self):
        sequence_values = super(Warehouse, self)._get_sequence_values()
        sequence_values.update({
            'clinic_type_id': {
                'name': self.name + ' ' + _('Picking Clinic'),
                'prefix': self.code + '/CLC/',
                'padding': 5,
    ...
    def _get_picking_type_update_values(self):
        ...
        picking_type_update_values.update({
            'clinic_type_id': {'default_location_src_id': self.lot_stock_id.id}
        })
    ...
    @api.model
    def _create_missing_clinic_picking_types(self):
        warehouses = self.env['stock.warehouse'].search([('clinic_type_id', '=', False)])
